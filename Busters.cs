...
class Player
{
    static void Main(string[] args)
    {
        ...
        int baseX = (myTeamId==0)?200:15800;
        int baseY = (myTeamId==0)?200:8800;
        ...
        while (true)
        {
            ...
            for (int bn = 0; bn < busPerPlayer; bn++)
            {
                ...
                {
                    minD = 100000;
                    if(bus[bn].state==0 && Enemy.Count>0)
                    {
                        Entity minE = Enemy[0];
                        Console.Error.WriteLine("Enemy.Count:{0}", Enemy.Count);
                        foreach(Entity o in Enemy)
                        if((D = (int)(Math.Pow(o.x - bus[bn].x,2)+Math.Pow(o.y - bus[bn].y,2)))<minD)
                        {
                            minD = D;
                            minE = o;
                            Console.Error.WriteLine("minD:{0} minE.Id:{1}", minD, minE.Id);
                        }
                        if(Math.Pow(minE.x - bus[bn].x,2)+Math.Pow(minE.y - bus[bn].y,2)<1760*1760)
